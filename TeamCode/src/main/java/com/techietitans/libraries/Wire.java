...
import com.techietitans.libraries.ArrayQueue;

import java.util.concurrent.locks.Lock;

/**
 * Created by ruhul on 12/31/15.
 */


/**
 * Based on an adruino library written by Olavi Kamppari on 10/25/2015.
 * This is emulating the Arduino Wire library used to communicate with I2C devices. Due to
 * reasonable long response times (5 - 15 ms) through Device Interface Module (DIM),
 ...
 * 3) added method requestCount(), to complement the existing responseCount()
 * 4) added method write() with second parameter as object (for no value)
 */
...
//------------------------------------------------- Add and Remove from Queue ------------------
    ...
    private void logCache(char cacheLetter, String function) {
        switch (cacheLetter) {
            case 'd':
                logCache(dCache, function, cacheLetter);
                break;
            case 'w':
                logCache(wCache, function, cacheLetter);
                break;
            case 'r':
                logCache(rCache, function, cacheLetter);
                break;
            case 'u':
                logCache(uCache, function, cacheLetter);
                break;
        }
    }
